...
type DefiToken {
    ...
    # canWrapGLXY signals if the token can be used
    # to wrap native GLXY tokens for DeFi trading.
    canWrapGLXY: Boolean!
...
type Delegation {
    ...
    # outstandingSGLXY represents the amount of sGLXY tokens representing
    # the tokenized stake minted and un-repaid on this delegation.
    outstandingSGLXY: BigInt!

    # tokenizerAllowedToWithdraw indicates if the stake tokenizer allows the stake
    # to be withdrawn. That means all the sGLXY tokens have been repaid and the sGLXY
...
type EstimatedRewards {
    # Amount of GLXY tokens expected to be staked for the calculation.
    staked: Long!

    # dailyReward represents amount of GLXY tokens estimated
    # to be rewarded for staked amount in average per day.
    dailyReward: BigInt!

    # weeklyReward represents amount of GLXY tokens estimated
    # to be rewarded for staked amount in average per week.
    weeklyReward: BigInt!

    # monthlyReward represents amount of GLXY tokens estimated
    # to be rewarded for staked amount in average per month.
    monthlyReward: BigInt!

    # yearlyReward represents amount of GLXY tokens estimated
    ...
    # Total amount of staked GLXY tokens used for the calculation in WEI units.
    ...
    # Information about the last sealed epoch of the GLXY blockchain.
    # The epoch provides useful information about total GLXY supply,
...
type Account {
    ...
    erc721TxList(cursor:Cursor, count:Int = 25, token: Address, tokenId: BigInt, txType: String): ERC721TransactionList!
...
type Query {
    ...
    # Total number of accounts active on the GLXY blockchain.
    accountsActive:Long!
    ...
    # Get the id of the current epoch of the GLXY blockchain.
    currentEpoch:Long!
    ...
    # The last staker id in GLXY blockchain.
    lastStakerId: Long!

    # The number of stakers in GLXY blockchain.
    stakersNum: Long!
    ...
    # Get price details of the GLXY blockchain token for the given target symbols.
    price(to:String!):Price!

    # Get calculated staking rewards for an account or given
    # staking amount in GLXY tokens.
    ...
    # by the Uniswap Core contract on GLXY blockchain.
