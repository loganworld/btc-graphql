...
import (
	"galaxychain-graphql/internal/repository/db"
...
func (p *proxy) DelegationOutstandingSGLXY(addr *common.Address, toStaker *hexutil.Big) (*hexutil.Big, error) {
	val, err := p.rpc.DelegationOutstandingSGLXY(addr, toStaker.ToInt())
	if err != nil {
		return nil, err
	}
	return (*hexutil.Big)(val), nil
