...
type Repository interface {
	// Account returns account at GLXY blockchain for an address, nil if not found.
	Account(*common.Address) (*types.Account, error)

	// AccountBalance returns the current balance of an account at GLXY blockchain.
	AccountBalance(*common.Address) (*hexutil.Big, error)

	// AccountNonce returns the current number of sent transactions of an account at GLXY blockchain.
	AccountNonce(*common.Address) (*hexutil.Uint64, error)

	// AccountTransactions returns list of transaction hashes for account at GLXY blockchain.
	//
	...
	// BlockHeight returns the current height of the GLXY blockchain in blocks.
	...
	// BlockByNumber returns a block at GLXY blockchain represented by a number.
	...
	// BlockByHash returns a block at GLXY blockchain represented by a hash.
	...
	// Contracts returns list of smart contracts at GLXY blockchain.
	...
	// LoadTransaction returns a transaction at GLXY blockchain
	// by a hash loaded directly from the node.
	LoadTransaction(hash *common.Hash) (*types.Transaction, error)

	// Transaction returns a transaction at GLXY blockchain by a hash, nil if not found.
	Transaction(*common.Hash) (*types.Transaction, error)

	// Transactions returns list of transaction hashes at GLXY blockchain.
	...
	// LastValidatorId returns the last validator id in GLXY blockchain.
	LastValidatorId() (uint64, error)

	// ValidatorsCount returns the number of stakers in GLXY blockchain.
	...
	DelegationOutstandingSGLXY(*common.Address, *hexutil.Big) (*hexutil.Big, error)
