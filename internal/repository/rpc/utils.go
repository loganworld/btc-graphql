...
func (glxy *GlxyBridge) GasPrice() (hexutil.Big, error) {
	// keep track of the operation
	glxy.log.Debugf("checking current gas price")

	// call for data
	var price hexutil.Big
	err := glxy.rpc.Call(&price, "glxy_gasPrice")
	if err != nil {
		glxy.log.Error("current gas price could not be obtained")
		return price, err
	}
...
func (glxy *GlxyBridge) GasEstimate(trx *struct {
	From  *common.Address
	To    *common.Address
	Value *hexutil.Big
	Data  *string
	...
	// keep track of the operation
	glxy.log.Debugf("calling for gas amount estimation")

	var val hexutil.Uint64
	err := glxy.rpc.Call(&val, "glxy_estimateGas", trx)
	if err != nil {
		// missing required argument? incompatibility between old and new RPC API
		if strings.Contains(err.Error(), "missing value") {
			return glxy.GasEstimateWithBlock(trx)
		}

		// return error
		glxy.log.Errorf("can not estimate gas; %s", err.Error())
		return nil, err
	}
...
func (glxy *GlxyBridge) GasEstimateWithBlock(trx *struct {
	From  *common.Address
	To    *common.Address
	Value *hexutil.Big
	Data  *string
	...
	// keep track of the operation
	glxy.log.Debugf("calling for gas amount estimation with block details")

	var val hexutil.Uint64
	err := glxy.rpc.Call(&val, "glxy_estimateGas", trx, BlockTypeLatest)
	if err != nil {
		// return error
		glxy.log.Errorf("can not estimate gas; %s", err.Error())
		return nil, err
