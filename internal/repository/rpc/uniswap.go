...
func (glxy *GlxyBridge) NativeTokenAddress() (*common.Address, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapRouter(glxy.uniswapConfig.Router, glxy.eth)
	if err != nil {
		glxy.log.Errorf("Uniswap router contract not found; %s", err.Error())
		return nil, err
	}
	...
	if err != nil {
		glxy.log.Errorf("Native token address not available; %s", err.Error())
		return nil, err
	}
...
func (glxy *GlxyBridge) UniswapPair(tokenA *common.Address, tokenB *common.Address) (*common.Address, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapFactory(glxy.uniswapConfig.Core, glxy.eth)
	if err != nil {
		glxy.log.Errorf("Uniswap factory contract not found; %s", err.Error())
		return nil, err
	}
	...
	if err != nil {
		glxy.log.Errorf("Uniswap pair not found for tokens %s and %s; %s", tokenA.String(), tokenB.String(), err.Error())
		return nil, err
	}
...
func (glxy *GlxyBridge) UniswapPairs(whiteListedOnly bool) ([]common.Address, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapFactory(glxy.uniswapConfig.Core, glxy.eth)
	if err != nil {
		glxy.log.Errorf("Uniswap factory contract not found; %s", err.Error())
	...
	if err != nil || length == nil {
		glxy.log.Error("Uniswap pairs array length not available")
		return nil, err
	}
	...
	for i := uint64(0); i < length.Uint64(); i++ {
		...
		if err != nil {
			glxy.log.Errorf("error loading Uniswap pair; %s", err.Error())
			continue
		}

		// check the pair (is it on the white list?)
		// add the address to the list, if it's ok
		if !whiteListedOnly || glxy.isUniswapPairWhitelisted(&adr) {
...
func (glxy *GlxyBridge) isUniswapPairWhitelisted(pair *common.Address) bool {
	// decode the pair address
	pairAddr := pair.String()

	// check the pair address against all the white listed pairs in config
	for _, addr := range glxy.uniswapConfig.PairsWhiteList {
...
func (glxy *GlxyBridge) UniswapQuoteInput(
	amountA hexutil.Big,
	...
	if err != nil {
		glxy.log.Errorf("can not calculate Uniswap quote; %s", err.Error())
		return hexutil.Big{}, err
	}
...
func (glxy *GlxyBridge) UniswapAmountsOut(amountIn hexutil.Big, tokens []common.Address) ([]hexutil.Big, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapRouter(glxy.uniswapConfig.Router, glxy.eth)
	if err != nil {
		glxy.log.Errorf("Uniswap router contract not found; %s", err.Error())
	...
	return glxy.uniswapLoadAmounts(contract.GetAmountsOut, (*big.Int)(&amountIn), tokens)
}
...
func (glxy *GlxyBridge) UniswapAmountsIn(amountOut hexutil.Big, tokens []common.Address) ([]hexutil.Big, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapRouter(glxy.uniswapConfig.Router, glxy.eth)
	if err != nil {
		glxy.log.Errorf("Uniswap router contract not found; %s", err.Error())
	...
	return glxy.uniswapLoadAmounts(contract.GetAmountsIn, (*big.Int)(&amountOut), tokens)
}

// uniswapLoadAmounts loads a list of swap amounts using the given loader function
// of the Uniswap router contract.
func (glxy *GlxyBridge) uniswapLoadAmounts(
	...
	if err != nil {
		glxy.log.Errorf("can not load swap amounts; %s", err.Error())
		return nil, err
	}
...
func (glxy *GlxyBridge) UniswapTokens(pair *common.Address) ([]common.Address, error) {
	// get the pair contract if possible
	contract, err := contracts.NewUniswapPair(*pair, glxy.eth)
	if err != nil {
		glxy.log.Errorf("Uniswap pair %s not found; %s", pair.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		glxy.log.Errorf("Uniswap pair %s token A not found; %s", pair.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		glxy.log.Errorf("Uniswap pair %s token B not found; %s", pair.String(), err.Error())
		return nil, err
	}
...
func (glxy *GlxyBridge) UniswapReserves(pair *common.Address) ([]hexutil.Big, error) {
	// get the reserves record from the contract
	rs, err := glxy.uniswapReservesRecord(pair)
	if err != nil {
		glxy.log.Errorf("Uniswap pair %s reserves not available; %s", pair.String(), err.Error())
		return nil, err
	}
...
func (glxy *GlxyBridge) UniswapReservesTimeStamp(pair *common.Address) (hexutil.Uint64, error) {
	// get the reserves record from the contract
	rs, err := glxy.uniswapReservesRecord(pair)
	if err != nil {
		glxy.log.Errorf("Uniswap pair %s reserves not available; %s", pair.String(), err.Error())
...
func (glxy *GlxyBridge) UniswapCumulativePrices(pair *common.Address) ([]hexutil.Big, error) {
	// get the pair contract if possible
	contract, err := contracts.NewUniswapPair(*pair, glxy.eth)
	if err != nil {
		glxy.log.Errorf("Uniswap pair %s not found; %s", pair.String(), err.Error())
	...
	if err != nil {
		glxy.log.Errorf("Uniswap pair %s cumulative price A not available; %s", pair.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		glxy.log.Errorf("Uniswap pair %s cumulative price B not available; %s", pair.String(), err.Error())
		return nil, err
	}
...
func (glxy *GlxyBridge) uniswapReservesRecord(pair *common.Address) (*struct {
	Reserve0           *big.Int
	Reserve1           *big.Int
...
func (glxy *GlxyBridge) UniswapLastKValue(pair *common.Address) (hexutil.Big, error) {
	// get the pair contract if possible
	contract, err := contracts.NewUniswapPair(*pair, glxy.eth)
	if err != nil {
		glxy.log.Errorf("Uniswap pair %s not found; %s", pair.String(), err.Error())
	...
	if err != nil {
		glxy.log.Errorf("Uniswap pair %s coefficient K not available; %s", pair.String(), err.Error())
		return hexutil.Big{}, err
	}
...
func (glxy *GlxyBridge) UniswapPairContract(pairAddres *common.Address) (*contracts.UniswapPair, error) {
	contract, err := contracts.NewUniswapPair(*pairAddres, glxy.eth)
	if err != nil {
		glxy.log.Errorf("Uniswap pair contract %s not found; %s", pairAddres.String(), err.Error())
		return nil, err
	}
	return contract, nil
}
...
func (glxy *GlxyBridge) UniswapFactoryContract() (*contracts.UniswapFactory, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapFactory(glxy.uniswapConfig.Core, glxy.eth)
	if err != nil {
		glxy.log.Errorf("Uniswap factory contract not found; %s", err.Error())
