...
func (glxy *GlxyBridge) GovernanceProposalsCount(gov *common.Address) (hexutil.Big, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, glxy.eth)
	if err != nil {
		glxy.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	if err != nil {
		glxy.log.Errorf("can not count governance %s proposals; %s", gov.String(), err.Error())
		return hexutil.Big{}, err
	}
...
// specified by its id.
func (glxy *GlxyBridge) GovernanceProposal(gov *common.Address, id *hexutil.Big) (*types.GovernanceProposal, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, glxy.eth)
	if err != nil {
		glxy.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	// get the detail
	gp, err := glxy.governanceProposalDetail(gc, gov, id.ToInt())
	if err != nil {
		glxy.log.Errorf("proposal %d not available in %s; %s", id.ToInt().Uint64(), gov.String(), err.Error())
		return nil, err
	}
...
// GovernanceProposal provides a detail of Proposal of a governance contract
// specified by its id.
func (glxy *GlxyBridge) governanceProposalDetail(gc *contracts.Governance, govId *common.Address, id *big.Int) (*types.GovernanceProposal, error) {
	...
	ext, err := glxy.GovernanceProposalDetails(&data.ProposalContract)
	if err != nil {
		return nil, err
	}
...
// specified by its id.
func (glxy *GlxyBridge) GovernanceProposalState(gov *common.Address, id *hexutil.Big) (*types.GovernanceProposalState, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, glxy.eth)
	if err != nil {
		glxy.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	if err != nil {
		glxy.log.Errorf("can not access governance %s proposal %d state; %s", gov.String(), id.ToInt().Int64(), err.Error())
		return nil, err
	}
...
// specified by its id.
func (glxy *GlxyBridge) GovernanceProposalDetails(prop *common.Address) (*govProposalExtended, error) {
	// get the proposal contract
	pp, err := contracts.NewGovernanceProposal(*prop, glxy.eth)
	if err != nil {
		glxy.log.Errorf("can not access governance proposal %s; %s", prop.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		glxy.log.Errorf("governance proposal %s name not available; %s", prop.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		glxy.log.Errorf("governance proposal %s description not available; %s", prop.String(), err.Error())
		return nil, err
	}
...
func (glxy *GlxyBridge) GovernanceOptionState(gov *common.Address, propId *hexutil.Big, optId *hexutil.Big) (*types.GovernanceOptionState, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, glxy.eth)
	if err != nil {
		glxy.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	gs, err := glxy.GovernanceOptionStateById(gc, propId, optId)
	if err != nil {
		glxy.log.Errorf("governance %s proposal #%d state #%d not available; %s",
...
func (glxy *GlxyBridge) GovernanceOptionStates(gov *common.Address, propId *hexutil.Big, optRange int) ([]*types.GovernanceOptionState, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, glxy.eth)
	if err != nil {
		glxy.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	for i := 0; i < optRange; i++ {
		...
		res[i], err = glxy.GovernanceOptionStateById(gc, propId, (*hexutil.Big)(big.NewInt(int64(i))))
		if err != nil {
			glxy.log.Errorf("unknown option #%d on governance %s; %s", i, gov.String(), err.Error())
			break
		}
	}
	return res, nil
...
func (glxy *GlxyBridge) GovernanceOptionStateById(gc *contracts.Governance, propId *hexutil.Big, optId *hexutil.Big) (*types.GovernanceOptionState, error) {
	// get the state
...
func (glxy *GlxyBridge) GovernanceVote(
	gov *common.Address,
	propId *hexutil.Big,
	from *common.Address,
	...
	if err != nil {
		glxy.log.Errorf("can not access vote of %s on governance %s; %s", from.String(), gov.String(), err.Error())
		return nil, err
	}
...
func (glxy *GlxyBridge) GovernanceProposalsBy(gov *common.Address) ([]*types.GovernanceProposal, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, glxy.eth)
	if err != nil {
		glxy.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	// log what we do
	glxy.log.Noticef("loading %d proposals of %s", maxProposalId.Uint64(), gov.String())
	...
	for i := maxProposalId.Int64(); i > 0; i-- {
		...
		gp, err := glxy.governanceProposalDetail(gc, gov, big.NewInt(i))
		if err != nil {
			glxy.log.Errorf("can not access governance %s proposal %d; %s", gov.String(), i, err.Error())
			return nil, err
		}

		// keep the proposal in the list
		glxy.log.Noticef("found proposal #%d on %s", gp.Id.ToInt().Uint64(), gov.String())
		result = append(result, gp)
	}

	// log what we do
	glxy.log.Debugf("%d proposals of %s loaded", maxProposalId.Uint64(), gov.String())
	return result, nil
}

// GovernanceProposalFee returns the fee payable for a new proposal
// in given Governance contract context.
func (glxy *GlxyBridge) GovernanceProposalFee(gov *common.Address) (hexutil.Big, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, glxy.eth)
	if err != nil {
		glxy.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	if err != nil {
		glxy.log.Errorf("governance %s fee not available; %s", gov.String(), err.Error())
		return hexutil.Big{}, err
	}
...
func (glxy *GlxyBridge) GovernanceTotalWeight(ge *common.Address) (*hexutil.Big, error) {
	// get the contract
	goe, err := contracts.NewGovernable(*ge, glxy.eth)
	if err != nil {
		glxy.log.Errorf("can not access governable adapter %s; %s", ge.String(), err.Error())
		return nil, err
	}

	// get the total available voting weight
	w, err := goe.GetTotalWeight(glxy.DefaultCallOpts())
	if err != nil {
		glxy.log.Errorf("total weight not available on adapter %s; %s", ge.String(), err.Error())
		return nil, err
	}
