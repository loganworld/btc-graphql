...
func (glxy *GlxyBridge) DefiTokens() ([]types.DefiToken, error) {
	// connect the contract
	contract, err := glxy.fMintCfg.tokenRegistryContract()
	if err != nil {
		return nil, err
	}

	return glxy.defiTokensList(contract)
}

// DefiTokenList creates a list of addresses / identifiers of all the ERC20 tokens
// involved with the fMint protocol.
func (glxy *GlxyBridge) DefiTokenList() ([]common.Address, error) {
	// connect the contract
	contract, err := glxy.fMintCfg.tokenRegistryContract()
	if err != nil {
		return nil, err
	...
	return glxy.defiTokenAddressList(contract.TokensCount, contract.TokensList)
}

// DefiToken loads details of a single DeFi token by it's address.
func (glxy *GlxyBridge) DefiToken(token *common.Address) (*types.DefiToken, error) {
	// connect the contract
	contract, err := glxy.fMintCfg.tokenRegistryContract()
	if err != nil {
		return nil, err
	...
	return glxy.defiTokenDetail(contract, token)
}
...
func (glxy *GlxyBridge) defiTokenAddressList(
	...
	if err != nil {
		glxy.log.Errorf("can not get tokens range; %s", err.Error())
		return nil, err
	}
	...
	for i := uint64(0); i < count.Uint64(); i++ {
		...
		if err != nil {
			glxy.log.Errorf("token %d address not found; %s", i, err.Error())
			return nil, err
		}
	}
...
func (glxy *GlxyBridge) defiTokenDetail(contract *contracts.DefiFMintTokenRegistry, token *common.Address) (*types.DefiToken, error) {
	// get the token details
	tk, err := contract.Tokens(nil, *token)
	if err != nil {
		glxy.log.Errorf("token %s not found; %s", token.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		glxy.log.Errorf("can not decode token %s; %s", token.String(), err.Error())
		return nil, err
	}
...
func (glxy *GlxyBridge) defiTokensList(contract *contracts.DefiFMintTokenRegistry) ([]types.DefiToken, error) {
	// get tge list of addresses
	al, err := glxy.defiTokenAddressList(contract.TokensCount, contract.TokensList)
	if err != nil {
		glxy.log.Errorf("tokens list not available; %s", err.Error())
		return nil, err
	}
	...
	for i, addr := range al {
		// decode the token
		tk, err := glxy.defiTokenDetail(contract, &addr)
		if err != nil {
			glxy.log.Errorf("invalid token #%d; %s", i, err.Error())
			return nil, err
		}
